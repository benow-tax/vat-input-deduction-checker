...
def _github_save_config(new_cfg):
    if not _HAS_REQUESTS:
        return False, "requests 라이브러리 없음"
    try:
        ...
        if r.status_code != 200:
            return False, f"파일 조회 실패: {r.status_code}"
        sha = r.json()["sha"]
        current = base64.b64decode(r.json()["content"]).decode("utf-8")

        # DEFAULT_CONFIG 섹션 교체
        marker_start = "DEFAULT_CONFIG = {"
        marker_end = "    'learned_nontax_reason': [],"
        s = current.find(marker_start)
        s = current.find(marker_start)
        e = current.find(marker_end)
        if s < 0 or e < 0:
            return False, "CONFIG 마커 없음"
        e += len(marker_end)

        cfg_lines = ["DEFAULT_CONFIG = {\n"]
        for k, v in new_cfg.items():
            cfg_lines.append(f"    {repr(k)}: {repr(v)},\n")
        cfg_lines.append("}")
        new_cfg_str = "".join(cfg_lines)
        updated = current[:s] + new_cfg_str + current[e:]

        encoded = base64.b64encode(updated.encode("utf-8")).decode("utf-8")
        payload = {
            "message": "설정 자동 업데이트 (앱에서 저장)",
            "content": encoded,
            "sha": sha
        }
        r2 = _requests.put(url, headers=headers, json=payload)
        if r2.status_code in [200, 201]:
            return True, "저장 완료"
        return False, f"저장 실패: {r2.status_code}"
...
def _github_save_config(new_cfg, st_obj):
    if not _HAS_REQ:
        return False, "requests 없음"
    try:
        token = st_obj.secrets.get("GITHUB_TOKEN", "")
        repo  = st_obj.secrets.get("GITHUB_REPO", "")
        fpath = st_obj.secrets.get("GITHUB_FILE", "app.py")
        if not token or not repo:
            return False, "Secrets 미설정"
...
with tab1:
    ...
    <div class="guide-box">
        <h4>키워드 직접 수정</h4>
        <p>검출 기준(키워드, 비고 문구 등)을 직접 수정할 수 있습니다.<br>
        수정 후 <strong>💾 설정 저장</strong> 버튼을 누르면 GitHub에 자동 저장되어 브라우저를 닫아도 영구적으로 유지됩니다.</p>
        <ul>
            <li>🔴 불공제 키워드 — 택시·대리운전·항공비·접대비 등 키워드 추가/삭제</li>
            <li>🟡 담당자확인 키워드 — 해외 매장·아지트·미국 아마존 관련 키워드 추가/삭제</li>
            <li>✅ 공제 예외 차량번호 — 경차 번호 추가/삭제</li>
            <li>📝 비고 문구 — 담당자확인 비고 내용 수정</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="guide-box">
        <h4>신규 패턴 학습</h4>
        <p>🆕 신규 패턴으로 감지된 항목을 검토 후 직접 공제/불공제로 등록할 수 있습니다.<br>
        한 번 등록하면 다음 검토부터 자동으로 처리되며, GitHub에 영구 저장됩니다.</p>
        <ul>
            <li>✅ <strong>공제 확정 등록</strong> — 신규 감지됐지만 공제 가능한 항목 등록 → 다음 검토부터 신규 감지에서 제외</li>
            <li>🔴 <strong>불공제 확정 등록</strong> — 신규 감지됐고 불공제로 확인된 항목 등록 → 다음 검토부터 자동 불공제 처리</li>
        </ul>
    </div>
    <div class="guide-box">
        <h4>⚠️ 저장 후 반영까지 1~2분 소요</h4>
        <p>설정 저장 또는 학습 등록 후 Streamlit이 자동 재배포되는 데 <strong>1~2분</strong>이 걸립니다.<br>
        저장 후 잠깐 기다렸다가 검토를 실행해 주세요.</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab3:
    ...
    if st.button('💾 설정 저장', type='primary', use_container_width=True):
        ...
        if ok:
            st.success('✅ 설정이 저장됐습니다! GitHub에 자동 반영되어 영구 저장됩니다.')
        else:
            st.success('✅ 현재 세션에 저장됐습니다. (GitHub 자동저장: ' + msg + ')')
        ...
        if st.button("✅ 공제 항목으로 등록", key="btn_exempt"):
            ...
            if kw:
                if kw not in learned_exempt:
                    ...
                    if ok:
                        st.success(f"'{kw}' 공제 항목으로 등록됐습니다! GitHub에 영구 저장됐습니다.")
                    else:
                        st.success(f"'{kw}' 공제 항목으로 등록됐습니다! (GitHub 자동저장: {msg})")
                    st.rerun()
                else:
        ...
        if st.button("🔴 불공제 항목으로 등록", key="btn_nontax"):
            ...
            if kw:
                if kw not in learned_nontax:
                    ...
                    ok, msg = _github_save_config(cfg2)
                    if ok:
                        st.success(f"'{kw}' 불공제 항목으로 등록됐습니다! GitHub에 영구 저장됐습니다.")
                    else:
                        st.success(f"'{kw}' 불공제 항목으로 등록됐습니다! (GitHub 자동저장: {msg})")
                    st.rerun()
                else:
                    st.warning("이미 등록된 키워드입니다.")
            else:
